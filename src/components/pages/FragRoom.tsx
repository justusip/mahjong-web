...
import ThreeSectionButton from "../generic/ThreeSectionButton";
...
export default function FragRoom(props: {
    ...
    if (!ctx.roomState) //TODO
        return <div></div>;

    const canStart = ctx.roomState.players.length === 4 && ctx.roomState.players.every((p: any) => p.ready);
    const iAmOwner = ctx.roomState.iAm === 0;
    const iAmReady = ctx.roomState.players[ctx.roomState.iAm].ready;
    return <Frag in={props.in}
        ...
        <div className="h-full w-[400px] flex flex-col">
            ...
            <div className="flex-1 flex flex-col gap-4 p-4 pr-0">
                {
                    [...new Array(4)].map((_, i) => {
                        const player = ctx.roomState.players[i];
                        if (!player) {
                        ...
                        return <div key={i} className="p-2 flex place-items-center bg-gray-700">
                            <div className="flex-1 flex flex-col">
                                <div className="text-xl">{ctx.roomState.players[i].name}</div>
